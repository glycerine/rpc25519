...
func Test408_multiple_circuits_open_and_close(t *testing.T) {
	...
	cli_lpb, err := j.cli.PeerAPI.StartLocalPeer(ctx, j.cliServiceName, "", nil, "peerName0", false)
	panicOn(err)
	defer cli_lpb.Close()

	srv_lpb, err := j.srv.PeerAPI.StartLocalPeer(ctx, j.srvServiceName, "", nil, "peerName1", false)
	panicOn(err)
	defer srv_lpb.Close()
	...
	assert(fragSrvInRead0.FragSubject == "initial setup frag0")
