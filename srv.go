...
var ErrContextCancelledPeer = fmt.Errorf("rpc25519 error: context cancelled: LocalPeer.")
var ErrContextCancelledCkt = fmt.Errorf("rpc25519 error: context cancelled: Circuit.")
...
func (s *rwPair) runSendLoop(conn net.Conn) {
	if s.cfg.UseSimNet {
		s.Server.simnode.newGoro("rwPair.runSendLoop")
	}
	stopReason := ""
	_ = stopReason
	defer func() {
...
func (s *Server) destAddrToSendCh(destAddr string) (sendCh *LoopComm, haltCh chan struct{}, to, from string, ok bool) {
	...
	if !ok {
		// do we know it by its base server name?
		destAddr1, destServerBaseName := autoCliToBaseURL(destAddr)
		...
		if !ok {
			...
			// to reach dest 'node_0', construct remote dest as
			// 'simnet://auto-cli-from-srv_node_0-to-srv_node_1'
...
func (s *Server) SendOneWayMessage(ctx context.Context, msg *Message, errWriteDur time.Duration, forbidAutoCli bool) (madeNewAutoCli bool, ch *LoopComm, err error) {
	...
	if err == ErrNetConnectionNotFound {
		...
		//vv("%v started auto-client ok. trying send again... from:'%v'; to:'%v'e", s.name, p.from, p.to)
		err, ch = sendOneWayMessage(s, ctx, msg, errWriteDur)
	}
	return
}
...
func (s *Server) ListClients() (remotes map[string]*Message, numRWPair int64) {
	...
	s.remote2pair.ReadOnlyView(func(m map[string]*rwPair) {
		for remote, pair := range m {
			_ = pair
			remotes[remote] = nil // pair.debugFirstReadMsg
		}
	})
	s.mut.Unlock()
	return
