...
func (s *rwPair) runSendLoop(conn net.Conn) {
	...
	w := newBlabber("server send loop", symkey, conn, s.Server.cfg.encryptPSK, maxMessage, true, true)
...
func (s *rwPair) runReadLoop(conn net.Conn) {
	...
	w := newBlabber("server read loop", symkey, conn, s.Server.cfg.encryptPSK, maxMessage, true, true)
