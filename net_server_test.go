...
func startNewServer() {
	...
	log.Println("NewNetServer test RPC server listening on", newServerAddr)
...
func testServeRequest(t *testing.T, server *NetServer) {
...
func (t *ArgNotPublic) ArgNotPublic(args *local2, reply *Reply) error {
	return nil
}

func (t *ReplyNotPublic) ReplyNotPublic(args *Args, reply *local2) error {
	return nil
}
...
func TestSendDeadlock(t *testing.T) {
	client := NewNetClientWithCodec(WriteFailCodec(0))
	defer client.Close()
...
func testSendDeadlock(client *NetClient) {
	defer func() {
		recover()
	}()
	args := &Args{7, 8}
...
func dialDirect() (*NetClient, error) {
	return Dial("tcp", serverAddr)
}

func dialHTTP() (*NetClient, error) {
	return DialHTTP("tcp", httpServerAddr)
}

func countMallocs(dial func() (*NetClient, error), t *testing.T) float64 {
	once.Do(startServer)
	client, err := dial()
	if err != nil {
...
func benchmarkEndToEnd(dial func() (*NetClient, error), b *testing.B) {
	once.Do(startServer)
	client, err := dial()
	if err != nil {
...
func benchmarkEndToEndAsync(dial func() (*NetClient, error), b *testing.B) {
	if b.N == 0 {
		return
	}
