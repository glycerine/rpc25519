...
func (s *TubeSim) handleAppendEntries(ae *AppendEntries, ckt0 *rpc.Circuit) (numOverwrote, numTruncated, numAppended int64) {
	...
	if !extends {
		...
		_, _, err := s.wal.overwriteEntries(keepCount, neededEntries, false, s.state.CommitIndex, s.state.LastApplied)
		panicOn(err)
		didAddData = true
	...
	if !done {
		...
		if extends {
			...
			default:
				...
				_, _, err := s.wal.overwriteEntries(keepCount, entries, false, s.state.CommitIndex, s.state.LastApplied)
				panicOn(err)
				didAddData = true
			}
		} else {
