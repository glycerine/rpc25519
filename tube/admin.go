...
func (s *TubeConfig) ShortSexpString(ps *zygo.PrintState) (r string) {
	...
	if s.BatchAccumulateDur != 0 {
		mainParts = append(mainParts, fmt.Sprintf(" BatchAccumulateDur: (dur %q)", s.BatchAccumulateDur))
	}
	if s.TupDefaultTable != "" {
		mainParts = append(mainParts, fmt.Sprintf("    TupDefaultTable: %q", s.TupDefaultTable))
	}
...
func (cfg *TubeConfig) ProdConfigSaneOrPanic(isReplica bool) {
	...
	if isReplica {
		...
		if cfg.BatchAccumulateDur < 0 || cfg.BatchAccumulateDur > time.Second*10 {
			panic("cfg.BatchAccumulateDur must be in the interval [0s, 10s]")
		}
	}
	if cfg.RpcCfg != nil {
