...
func (s *raftWriteAheadLog) overwriteEntries_NODISK(keepIndex int64, es []*RaftLogEntry, isLeader bool, curCommitIndex, lastAppliedIndex, keepCount, overwriteCount int64) (compactPrevIndex, compactPrevTerm int64, err error) {
	...
	if !s.noLogCompaction {
		...
		compactPrevIndex, compactPrevTerm = s.maybeCompact(curCommitIndex)
	}
