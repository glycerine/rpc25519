...
func Test401_add_node(t *testing.T) {
	...
	onlyBubbled(t, func(t *testing.T) {
		...
		for numNodes := minClusterSz; numNodes < maxClusterSz; numNodes++ {
			...
			list0, _, _, _, _, _, _, err := node4.GetPeerListFrom(bkg, leaderURL, leaderNode.name)
			panicOn(err)
			...
			list1, _, _, _, _, _, _, err := node4.GetPeerListFrom(bkg, leaderURL, leaderNode.name)
			panicOn(err)
			...
			list5pre, _, _, _, _, _, _, err := node5.GetPeerListFrom(bkg, leaderURL, leaderNode.name)
			panicOn(err)
			...
			list5, _, _, _, _, _, _, err := node5.GetPeerListFrom(bkg, leaderURL, leaderNode.name)
			panicOn(err)
			...
			_, _, err = node4.UseLeaderURL(bkg, leaderNode.name, leaderURL)
			panicOn(err)
			...
			list2, _, _, _, _, _, _, err := c.Nodes[1].GetPeerListFrom(bkg, leaderURL, leaderNode.name) // got to here
			panicOn(err)
