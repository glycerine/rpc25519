...
func Test401_add_node(t *testing.T) {
	...
	onlyBubbled(t, func(t *testing.T) {
		...
		for numNodes := minClusterSz; numNodes < maxClusterSz; numNodes++ {
			...
			list1, _, _, _, _, _, err := node4.GetPeerListFrom(bkg, leaderURL, leaderNode.name)
			panicOn(err)
			...
			list5, _, _, _, _, _, err := node5.GetPeerListFrom(bkg, leaderURL, leaderNode.name)
			panicOn(err)
			...
			panicOn(err)
			tkt, err := node4.Write(bkg, "", "xmem", xmem.SerzBytes(), 0, nil, "", 0)
			panicOn(err)
			vv("tkt = '%v'", tkt)
			...
			list2, _, _, _, _, _, err := c.Nodes[1].GetPeerListFrom(bkg, leaderURL, leaderNode.name) // got to here
			panicOn(err)
