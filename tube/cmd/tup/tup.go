...
func leaseInfoFromLeaf(leaf *art.Leaf) string {
	...
	return fmt.Sprintf(" %v   Leasor:'%v'; LeaseEpoch='%v' (T0:%v ago at %v);  LeaseUntilTm='%v' %v (leaf.Version='%v'; auto-delete: %v; LeaseRenewalElap: '%v')", nice(time.Now()), leaf.Leasor, leaf.LeaseEpoch, time.Since(leaf.LeaseEpochT0), nice(leaf.LeaseEpochT0), nice(leaf.LeaseUntilTm), lefts, leaf.Version, leaf.AutoDelete, leaf.LeaseRenewalElap)
}
