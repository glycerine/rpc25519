...
func main() {
	...
	var rsyncPath = flag.String("rsync", "", "path to rsync up to server. We act as an rsync sender (of files/file diff chunks); srv -rsync will tells us their current chunks and receive the needed diffs from us.")

	var pullRsync = flag.Bool("pull", false, "rsync pull data to remote (default is push")
	...
	var certPath = flag.String("certs", certdir, "use this path on the lived filesystem for certs; instead of the embedded certs/ from build-time.")
