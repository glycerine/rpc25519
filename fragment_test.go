...
func Test400_Fragments_riding_Circuits_API(t *testing.T) {
	...
	peerURL_client, peerID_client, _, _, err := srv.PeerAPI.StartRemotePeer(
		ctx, cliServiceName, "", cliAddr, 50*time.Microsecond, false)
	...
	lpb, err := srv.PeerAPI.StartLocalPeer(ctx, srvServiceName, "", nil, "peerName3", false)
	panicOn(err)
	defer lpb.Close()
	...
	assert(cpeer0.StartCount.Load() == 1)
	assert(speer1.StartCount.Load() == 2)
	*/
...
func Test401_PeerURL_parsing(t *testing.T) {
	...
	assert(netAddr == "tcp://x.x.x.x:5023")
	assert(serviceName == "serviceName")
	assert(peerID == "peerID")
	assert(circuitID == "circuitID")
...
func Test402_simpler_startup_peer_service_test(t *testing.T) {
	...
	lpb, err := srv.PeerAPI.StartLocalPeer(ctx, srvServiceName, "", nil, "peerName0", false)
	panicOn(err)
	peerURL_server, peerID_server := lpb.URL(), lpb.PeerID
	vv("StartLocalPeer: on server peerURL_server = '%v'; peerID_server = '%v'", peerURL_server, peerID_server)
	if cfg.UseQUIC {
		assert(strings.HasPrefix(peerURL_server, "udp://"))
	} else {
		assert(strings.HasPrefix(peerURL_server, "tcp://"))
	}
...
func Test403_new_circuit_from_existing_peer(t *testing.T) {
	...
	lpb, err := srv.PeerAPI.StartLocalPeer(ctx, srvServiceName, "", nil, "peerName2", false)
	peerURL_server, peerID_server := lpb.URL(), lpb.PeerID
	panicOn(err)
