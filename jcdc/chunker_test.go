...
Looking at the code, I can help clarify the relationship between these different approaches:
The rolling.go and chunker.go implementation you have is actually a different chunking algorithm altogether - it implements a simple exponential Content-Defined-Chunking algorithm that uses a rolling hash to find cut points. This is more similar to the original Rabin-Karp style chunking.
...
func TestChunker(t *testing.T) {
	...
	chunker := chunker.New(1024, 64, 8192)

	// Create a rolling hash with some seed
	roller := chunker.NewRollingHash(1)
