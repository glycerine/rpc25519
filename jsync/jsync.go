...
func newJsyncU(u rpc.UniversalCliSrv, serviceName string, isCli bool) (j *jsyncU, err error) {
	...
	localPeerName := fmt.Sprintf("jsyncU_%v_%v", serviceName, cryRandBytesBase64(15))
	lpb, err := u.StartLocalPeer(ctx, serviceName, "", nil, localPeerName, false)
	panicOn(err)
	j.lpb = lpb
...
jobDone:
	for {
		select { // hung here on 220 and 440
		case prog := <-req.UpdateProgress:
