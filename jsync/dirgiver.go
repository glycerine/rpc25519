	...
	for {
		...
		case frag0 := <-ckt.Reads:
			...
			case OpRsync_TakerRequestsDirSyncBegin: // 21
				...
				panicOn(err)
				begin.Payload = bts
				_, err = ckt.SendOneWay(begin, 0, 0)
				panicOn(err)
				bt.bsend += len(bts)
			...
			sendOnePass:
				for i := 0; ; i++ {
					...
					case pof := <-packOfFilesCh:
						...
						_, err = ckt.SendOneWay(fragPOF, 0, 0)
						panicOn(err)
						bt.bsend += len(bts)
...
func (s *SyncService) convertedDirToFile_giveFile(
	...
	_, err := ckt.SendOneWay(tofile, 0, 0)
	panicOn(err)
	...
	syncReq := reqDir.SR
	err = s.giverSendsPlanAndDataUpdates(&Chunks{Path: takerPath}, ckt, giverPath, bt, frag0, syncReq)
	panicOn(err)
