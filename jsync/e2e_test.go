...
func Test221_unwritten_pre_allocated_transfer(t *testing.T) {

	cv.Convey("verify pre-allocated/sparse file transfer on a single file", t, func() {
		...
		localDir := cwd + sep + "local_cli_dir_test221/"
		remoteDir := cwd + sep + "remote_srv_dir_test221/"
		// remove any old leftover test files first.
		os.RemoveAll(localDir)
		os.RemoveAll(remoteDir)
		os.MkdirAll(localDir, 0755)
		...
		vv("testing transfer of mypath '%v'", mypath)
