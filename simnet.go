...
iloop:
	for i := int64(0); ; i++ {
		...
		//griddur, gridgoal := s.durToGridPoint(now, s.scenario.tick)
		...
		for {
			...
			if left < 0 {
				panicf("should never overrun our igridtm(%v)! now = %v", nice9(igridtm), nice9(now))
			}
			// INVAR: left > 0
...
func (s *Simnet) distributeMEQ(now time.Time, i int64) (npop int, restartNewScenario, shutdown bool) {
	...
	if npop == 0 {
		...
		for s.curSliceQ.Len() > 0 {
			...
			vv("in distributeMEQ, [issueOrder:%v] curSliceQ has op = '%v'\n  ->  xdis = '%v'", s.xissueLast, op, xdis)
